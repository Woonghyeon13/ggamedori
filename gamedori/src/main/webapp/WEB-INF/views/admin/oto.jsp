...
<main>
	...
	<div class="container mt-1">
		...
		function sendAjaxRequest(searchText, searchOption, page, onSuccess) {
		    $.ajax({
		        url: '<%=request.getContextPath()%>/admin/otoSearch.do',
		        method: 'GET',
		        dataType: 'json',
		        data: {
	      ...
	      //  $.each(originalTableData, function (index, row) {
	       ...
	       // });
			...
			if (result.qa_yn === 1){
			 ...
			 // 답변 버튼 추가
			 // var answerButton = $('<button type="button" class="btn btn-secondary btn-sm"
				//		data-bs-toggle="modal" data-bs-target="#otoRefund" 
			//				onclick="sessionToModal('${otol.qa_idx }', '${otol.qa_title }',
				//			 '${otol.qa_contents }', '${otol.member_name }', '${otol.qa_reply }')" >').text('답변');
